describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    // Query select first <product-item> element
    const value = await page.$eval('product-item', item => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItemsData = await page.$$eval('product-item', prodItems => {
      return prodItems.map(item => {
        ...
        return document.querySelector("#cart-count").innerHTML;
      });
    });

    const value = await page.$eval('#cart-count', item => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let allArePopulated = 1;
    for (let i = 0; i < prodItemsData.length; i++)
      allArePopulated &= prodItemsData[i];
    expect(allArePopulated).toBe(1);
